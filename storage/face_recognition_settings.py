...
def add_picture_to_dataset(dataset: list, image: np.array, filename: str):
    ...
    for [saved_image, _], [_, _] in dataset:
        if type(saved_image == image) != bool and (saved_image == image).all():
...
def python_work(images, face_encodings_target, length=10):
    exclude_indexes = []
    ...
    while len(ten_nearest_images_with_coefs) < length:
        ...
        ten_nearest_images_with_coefs.update(
            {filename_of_minimum_image: minimum_distance})
